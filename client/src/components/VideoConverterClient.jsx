// client/src/components/VideoConverter.jsx
import React, { useState, useRef } from 'react';

const VIDEO_OPTS = ['mp4', 'webm'];
...
export default function VideoConverter({ onBack }) {
    ...
    async function convertOne(item) {
        try {
            ...
            const resp = await fetch(`${API_BASE}/api/convert/video`, {
                method: 'POST',
                body: fd
            });
    ...
    return (
        <div style={{ padding: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <h2 style={{ margin: 0 }}>Video Converter</h2>
                    <p style={{ margin: '6px 0 0', color: '#6b7280' }}>
                        Upload small video files (MP4 / MOV). Choose output after adding to queue.
                    </p>
                </div>
                <div>
                    <button
            ...
            <div style={{ marginTop: 18, background: '#fff', padding: 12, borderRadius: 12, border: '1px solid #eee' }}>
                ...
                <div style={{ marginTop: 8, color: '#9ca3af' }}>
                    Video conversion is processed on the server for each file.
                </div>
            </div>
        </div>
    );
